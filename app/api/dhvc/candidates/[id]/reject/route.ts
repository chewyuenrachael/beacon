...
import { dhvcIdParamsSchema, rejectCandidateBodySchema } from "../../../schemas";

const TERMINAL_STAGES = new Set(["accepted", "rejected", "archived"]);

/**
 * POST /api/dhvc/candidates/[id]/reject
 *
 * Transitions `review_stage` to `rejected`. Logs `dhvc_candidate_rejected`
 * with optional reason payload.
 *
 * Errors:
 *   VALIDATION           400
 *   NOT_FOUND            404
...
export async function POST(
  ...
  const parsedBody = rejectCandidateBodySchema.safeParse(json);
  ...
  try {
    ...
    if (candidate.review_stage === "rejected") {
      return NextResponse.json(candidate);
    }
    if (TERMINAL_STAGES.has(candidate.review_stage)) {
      return NextResponse.json(
        {
          error: `Cannot reject candidate already in ${candidate.review_stage}`,
    ...
    await logObservation({
      ...
      entity_id: id,
      observation_type: "dhvc_candidate_rejected",
      payload: {
    ...
    console.error("[api/dhvc/candidates/[id]/reject POST]", e);
    return NextResponse.json(
