...
export async function POST() {
  try {
    const mentions = await ingestHackerNews();
    ...
    console.error("POST /api/ingest/hackernews error:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "HN ingestion failed" },
      { status: 500 }
    );
  }
}
